...
public class DetailViewController implements Initializable {
	...
	@FXML
	private Label masterIP;
	@FXML
	private Label targetIP;
			...
			for (int i = 0; i < MainApplicationController.deviceData.size(); i++) {
				if (name.equals(MainApplicationController.deviceData.get(i).getPcName())) {
					...
					if (Device.existSub && type.equals("Subscriber")) { // 한개의 subscriber가 이미 작동중이고 그 후에 또 subscriber
																		// 선택시
						errorMessage = "One subscriber is already in operation.";

						Alert alert = new Alert(AlertType.ERROR);
						Stage curStage = stage;
						alert.initOwner(curStage);
						alert.setTitle("Warning!");
						alert.setHeaderText("Failed to specify client type");
					...
					if (app.isConnect) {
						MainApplicationController.deviceData.get(i).isConnect = true;

						// start 버튼 누를시 다른 필드들 수정 못하게, 데이터 저장//
						saveData(MainApplicationController.deviceData.get(i));
						// start , stop 버튼 색 바꾸기
						...
						alert.setContentText("Connection to test load broker failed. Please try again.");
	...
	public void stopAction(ActionEvent action) {
		...
		for (int i = 0; i < MainApplicationController.deviceData.size(); i++) {
			...
			if (name.equals(MainApplicationController.deviceData.get(i).getPcName())) {
				...
				if (mainApplicationController.deviceData.get(i).isStart) {
				...
				MainApplicationController.deviceData.get(i).isStart = false; 
				MainApplicationController.deviceData.get(i).isConnect = false; 
		...
		System.out.println(stopBtn.disabledProperty());
	...
	public boolean isCbxInputValid() {
		...
		if (errorMessage.length() == 0) {
			...
			alert.initOwner(curStage);
			alert.setTitle("Warning!");
			alert.setHeaderText("Empty options");
			alert.setContentText(errorMessage);
	...
	public boolean topicListValid() {
		...
		if (errorMessage.length() == 0) {
			...
			alert.initOwner(curStage);
			alert.setTitle("Warning!");
			alert.setHeaderText("Empty topics");
			alert.setContentText(errorMessage);
		...
		for (int i = minNum; i <= maxNum; i++) {
			if (type.equals("Publisher")) {
				if (Device.clientNumIndex.contains(i)) {
					...
					alert.initOwner(curStage);
					alert.setTitle("Warning!");
					alert.setHeaderText("Overlapping range");
					alert.setContentText("Duplicate client range exists.Please check it.");
	...
	public void saveData(Device device) {
		for (int i = 0; i < MainApplicationController.deviceData.size(); i++) {
			if (name.equals((device.getPcName()))) {
				device.setClientType(typeCbx.getSelectionModel().getSelectedItem());
				device.setCreateOption(optionCbx.getSelectionModel().getSelectedItem());
				device.setPayloadSize(payloadCbx.getSelectionModel().getSelectedItem());

				device.setDuration(Integer.parseInt(durationTf.getText()));
				device.setMinClientNum(Integer.parseInt(minTf.getText()));
				device.setMaxClientNum(Integer.parseInt(maxTf.getText()));

				if (device.getClientType().equals("Publisher")) {
					device.setTopicNum(Integer.parseInt(topicNumTf.getText()));
					device.setMsgRate(Integer.parseInt(msgRateTf.getText()));
				}

				device.setTopics(selected);

				device.saved();

				device.setStartBtn(startBtn);
				device.setStopBtn(stopBtn);
				device.setDurationTf(durationTf);
				device.setMaxTf(maxTf);
				device.setMinTf(minTf);
				device.setMsgRateTf(msgRateTf);
				device.setTopicNumTf(topicNumTf);
				device.setOptionCbx(optionCbx);
				device.setPayloadCbx(payloadCbx);
				device.setTypeCbx(typeCbx);
